...
export default function ChatPanel({ campaigns }) {
  ...
  const tabs = [
    ...
    ...(isAgency ? [{ id: "conseils", label: "Conseils", icon: Lightbulb }] : []),
          ...
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${
                activeTab === tab.id
                  ? tab.id === "paiement" ? "bg-[#f0c040]/20 text-[#f0c040]"
                  ...
                  {!paymentSummary ? (
                    ...
                    <>
                      <div className="bg-[#f0c040]/10 border border-[#f0c040]/20 rounded-xl p-5 text-center">
                        ...
                        <p className="text-[#f0c040] font-mono font-black text-4xl">€{paymentSummary.earned?.toFixed(2)}</p>
